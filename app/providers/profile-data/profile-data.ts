...
export class ProfileData {
  ...
  */
  updateName(firstName: string, lastName: string): any {
    return this.userProfile.child(this.currentUser.uid).update({
      firstName: firstName,
      lastName: lastName,
    }).then(()=> this.authData.logUserActivity('Update Name'));
  }
  ...
  */
  updateDOB(birthDate: string): any {
    return this.userProfile.child(this.currentUser.uid).update({
      birthDate: birthDate,
    }).then(()=> this.authData.logUserActivity('Update birth day'));;
  }
  ...
  updateEmail(newEmail: string): any {
    this.currentUser.updateEmail(newEmail).then(() => {
      this.userProfile.child(this.currentUser.uid).update({
        email: newEmail
      }).then(()=> this.authData.logUserActivity('Update email'));;
    }, (error) => {
      console.log(error);
    });
  }
  ...
  */
  updatePassword(newPassword: string): any {
    this.currentUser.updatePassword(newPassword).then(() => {
      ...
      this.authData.logUserActivity('Update password');
    }, (error) => {
      console.log(error);
    });
  }
